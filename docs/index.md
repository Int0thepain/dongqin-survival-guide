---
layout: home

hero:
  name: 东秦生存手册
  text: 东北大学秦皇岛校区新生生活指南
  tagline: 从报到入学到宿舍食堂，从校园网到选课攻略，帮你快速适应校园生活，持续更新的实用手册。
  image:
    src: /images/campus-map-official.jpg
    alt: 校园地图
  actions:
    - theme: brand
      text: 🚀 新生入口
      link: /02-报到入学
    - theme: alt
      text: 🏠 宿舍速查
      link: /13-宿舍详解与选择建议
    - theme: alt
      text: 📖 全书阅读
      link: /book-print
    - theme: alt
      text: 📥 下载 PDF
      link: /downloads/dongqin-survival-guide.pdf

features:
  - icon: 📋
    title: 入学篇
    details: 报到材料、到校交通、军训安排、校园卡与一网通办，助你顺利入学。
  - icon: 🏠
    title: 生活篇
    details: 宿舍分配、食堂测评、快递洗浴、校医院打印，校园生活全覆盖。
  - icon: 🤝
    title: 共建篇
    details: 保留来源、标注更新时间，持续补充真实经验和常见问题。
  - icon: 📚
    title: 全书版
    details: 自动合并章节，支持连续阅读、浏览器打印和 PDF 下载。
---

<p class="mobile-quick-title">手机快速入口</p>

<div class="mobile-quick">
  <a href="./02-报到入学">报到入学</a>
  <a href="./13-宿舍详解与选择建议">宿舍</a>
  <a href="./05-饮食指南">食堂</a>
  <a href="./03-军训与智慧校园">校园卡/军训</a>
  <a href="./14-学习选课与成绩FAQ">选课成绩</a>
  <a href="./15-生活常见问题">生活 FAQ</a>
  <a href="./16-校园社群与信息渠道">信息渠道</a>
  <a href="./TODO-可补充内容">待补充</a>
</div>

<div class="mobile-bottom-actions">
  <a href="./book-print">全书</a>
  <button type="button" onclick="document.querySelector('.DocSearch-Button')?.click()">搜索</button>
  <a href="./downloads/dongqin-survival-guide.pdf">PDF</a>
</div>

## 📌 当前状态

本项目目前是从《东北大学(秦皇岛)2026届新生入学生活指南.pdf》转换整理出的基础版。后续可以继续补充班级经验、专业经验、选课考试、竞赛科研、保研考研和实习就业等内容。

<div class="notice-danger">
<h2>⚠️ 免责声明</h2>
<p>请注意，本手册为<strong>非官方内容整理</strong>，主要来源于公开资料、补充材料和同学经验记录，仅供学习、生活参考。由于学校政策、系统入口、收费标准、宿舍安排、课程要求等内容可能随时间调整，手册内容存在过时、遗漏或理解偏差的风险。</p>
<p>广大同学应优先以<strong>学校、学院、辅导员、教务系统、迎新系统</strong>等正式通知和要求为准。本手册不鼓励、不建议任何违反学校规定、法律法规或公共秩序的行为；如手册内容与官方规定不一致，请一律以官方规定为准，并欢迎协助反馈修正。</p>
</div>

## ✨ 维护原则

- 🏛️ 官方信息注明来源，以学校最新通知为准
- 📝 经验内容标注适用范围，避免把个人体验写成绝对结论
- 🔄 时间、金额、电话、地点每学期开学前集中校对

## 📞 联系我们

如果你想补充内容、纠错，或提供新的经验材料，欢迎通过以下方式反馈：

<div class="contact-cards">

| 渠道 | 链接 |
| --- | --- |
| 📬 GitHub Issues | [提交反馈或补充](https://github.com/Int0thepain/dongqin-survival-guide/issues) |
| 💻 GitHub 仓库 | [查看网站源码](https://github.com/Int0thepain/dongqin-survival-guide) |
| 💬 QQ 联系 | `3010383177`（请备注“东秦生存手册 + 来意”） |

</div>

> ⚠️ **隐私提醒**：为避免个人隐私泄露，请不要在公开 Issue、公开页面或陌生私聊中提交身份证号、学号、手机号、宿舍号等敏感信息。
